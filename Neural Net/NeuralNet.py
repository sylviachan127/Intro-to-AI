...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """
        newActs = [1.0] + inActs
        newSum = self.getWeightedSum(newActs)
        return self.sigmoid(newSum)
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        newActs = [1.0] + inActs
        weight = self.getWeightedSum(newActs)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for w in self.weights:
            # w <- w + alpha*a*delta(j)
            newWeight = alpha*newActs[x]*delta
            self.weights[x] = w + newWeight
            totalModification += abs(newWeight)
            x+=1
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        """
        forward = []
        forward.append(inActs)
        currentAct = inActs
        for l in self.layers:
            newAct = []
            for nodes in l:
                newAction = nodes.sigmoidActivation(currentAct)
                newAct.append(newAction)
            currentAct = newAct
            forward.append(newAct)
        return forward
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for i in xrange(len(nextLayer)):
                        delta = deltas[0][i]
                        next = nextLayer[i].weights[neuronNum+1]
                        totalSum+=delta*next
                    delta = gPrime * totalSum
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for e in examplesTest:
        exampleOut = e[1]
        feedResult = nnet.feedForward(e[0])
        feedOutputList = feedResult[-1]
        accurate = True
        x = 0
        while x < len(feedOutputList):
            if round(feedOutputList[x]) != exampleOut[x]:
                accurate = False
            x+=1
        if not accurate:
            testError+=1
