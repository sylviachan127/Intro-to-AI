...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0: 
                updatedBelief[p] = emissionModel[trueDistance] * self.beliefs[p]
        if noisyDistance == None:
            updatedBelief = util.Counter()
            updatedBelief[self.getJailPosition()] = 1
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        updatedBelief = util.Counter()
        #oldPos = previous ghost position
        # newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)) # distribution over new positions for the
                                                                    #ghost, given its previous position (oldPos) as well as Pacman's current position
        for p in self.legalPositions:
            # self.beliefs:  {(2, 7): 0.058823529411764705, (4, 7): 0.058823529411764705, (1, 3): 0.058823529411764705, (3, 7): 0.058823529411764705, (3, 3): 0.058823529411764705, (5, 6): 0.058823529411764705, (5, 5): 0.058823529411764705, (5, 7): 0.058823529411764705, (5, 4): 0.058823529411764705, (1, 4): 0.058823529411764705, (1, 5): 0.058823529411764705, (2, 3): 0.058823529411764705, (4, 3): 0.058823529411764705, (1, 7): 0.058823529411764705, (1, 6): 0.058823529411764705, (5, 3): 0.058823529411764705, (3, 5): 0.058823529411764705}
            # print "oldPos: ", self.beliefs[p]
            # oldPos = self.beliefs[p]
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            # print "newPosDist: ", newPosDist newPosDist:  {(5.0, 4.0): 0.3333333333333333, (4.0, 3.0): 0.3333333333333333, (5.0, 3.0): 0.3333333333333333}
            for newPos, newProp in newPosDist.items():
                # print "updatedBelief: ", updatedBelief
                updatedBelief[newPos] += newProp*self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # actualParticle = self.numParticles/positionTotal
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0: 
                    ...
                    updatedBelief[p] = emissionModel[trueDistance] * prior[p] # Give weights to particle
            if not any(updatedBelief.values()):
                self.initializeUniformly(gameState)
            else:
                totalParticle = 0
                while totalParticle<self.numParticles:
                    selfParticle.append(util.sample(updatedBelief))
                    totalParticle+=1
                self.particles = selfParticle
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            sampleParticle = util.sample(newPosDist)
            newSample.append(sampleParticle)
    ...
    def getBeliefDistribution(self):
        ...
        beliefDistribute = util.Counter()
        for x in self.particles:
            beliefDistribute[x] += 1
        beliefDistribute.normalize()
        return beliefDistribute
